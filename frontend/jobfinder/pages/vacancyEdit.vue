            ...
            <section>
              <div v-if="permission">
                <header class="section-header">
                  ...
                  <p>
                    Пожалуйста, проверьте информацию еще раз и нажмите нужную
                    кнопку ниже для редактирования вакансии.
                  </p>
                </header>
...
export default {
  ...
  methods: {
    ...
    async deleteVacancy(event) {
      ...
      console.log(headers);
      await axios
        .delete(`${baseUrl()}/vacancyapp/${this.$route.query.id}/`, { headers })
        .then((response) => {
          this.vacancy = response.data;
        })
        .catch((error) => console.log(error));
